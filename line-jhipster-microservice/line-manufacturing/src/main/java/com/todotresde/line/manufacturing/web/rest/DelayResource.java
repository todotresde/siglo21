...
public class DelayResource {
    ...
    @GetMapping("/delays")
    @Timed
    public ResponseEntity<List<Delay>> getAllDelays(@ApiParam Pageable pageable) {
        log.debug("REST request to get a page of Delays");
        Page<Delay> page = delayService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/delays");
