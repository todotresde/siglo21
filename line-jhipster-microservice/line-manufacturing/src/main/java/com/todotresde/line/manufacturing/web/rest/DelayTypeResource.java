...
public class DelayTypeResource {
    ...
    @Timed
    public ResponseEntity<List<DelayType>> getAllDelayTypes(@ApiParam Pageable pageable) {
        log.debug("REST request to get a page of DelayTypes");
        Page<DelayType> page = delayTypeService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/delay-types");
