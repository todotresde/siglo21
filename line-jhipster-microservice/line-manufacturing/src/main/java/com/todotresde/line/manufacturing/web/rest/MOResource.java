...
public class MOResource {
    ...
    @GetMapping("/m-os")
    @Timed
    public ResponseEntity<List<MO>> getAllMOS(@ApiParam Pageable pageable) {
        log.debug("REST request to get a page of MOS");
        Page<MO> page = mOService.findAll(pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/m-os");
